...
, [categoryPriceRankWRank] = RANK() OVER(PARTITION BY c.[Name] ORDER BY a.[ListPrice] DESC)
, [categoryPriceRankWDenseRank] = DENSE_RANK() OVER(PARTITION BY c.[Name] ORDER BY a.[ListPrice] DESC)
, [top5PriceInCategory] = case when DENSE_RANK() OVER(PARTITION BY c.[Name] ORDER BY a.[ListPrice] DESC) <=5 then 'Yes' else 'No' end
